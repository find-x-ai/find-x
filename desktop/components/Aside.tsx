    ...
    <div className="h-screen flex flex-col justify-between px-10 py-5 bg-black border-r border-zinc-900">
      <div className="flex flex-col text-start gap-3 text-[#fff]">
        <Link
          className={`hover:bg-zinc-900 transition-colors duration-300 py-2 px-3 rounded-full flex items-center gap-3 ${
            path === "/" ? "text-[#ff371a] bg-zinc-950 " : "text-[#fff]"
          }`}
          href={"/"}
        >
          <HomeIcon
            className={`${path == "/" ? "fill-[#ff371a]" : "fill-white/90"}`}
          />
          Home
        </Link>
        <Link
          className={`hover:bg-zinc-900 transition-colors duration-300 py-2 px-3 rounded-full flex items-center gap-3 ${
            path === "/requests" ? "text-[#ff371a] bg-zinc-950" : "text-[#fff]"
          }`}
          href={"/requests"}
        >
          <Radio />
        ...
        <Link
          className={`hover:bg-zinc-900 transition-colors duration-300 py-2 px-3 rounded-full flex items-center gap-3 ${
            path === "/all" ? "text-[#ff371a] bg-zinc-950" : "text-[#fff]"
          }`}
          href={"/all"}
        >
          <AllClients
            className={`${path == "/all" ? "fill-[#ff371a]" : "fill-white/90"}`}
          />
          clients
        </Link>
      </div>
      <div className="flex flex-col gap-5">
        ...
        <div className="flex flex-col gap-1 text-sm">
          <span className="text-white">v0.1.6</span>
...
const AllClients = ({ className }: { className: string }) => {
  return (
    <svg
      viewBox="0 0 24 24"
      aria-hidden="true"
